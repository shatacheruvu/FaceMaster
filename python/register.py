...
# ==============================================================================
''' Attendance System based on Transfer Learning on Google's Inception V3 model
    Train your model based on the instructions given here:
...
def get_labels():
    ...
    with open('output_labels.txt', 'r') as file:
        labels = [line.rstrip('\n') for line in file]
    return labels
...
def run_detection(imageArray, labels):

    with tf.gfile.FastGFile('output_graph.pb', 'rb') as graph_file:
    ...
    with tf.Session() as sess:
        ...
        predictions = sess.run(softmax_tensor, {'DecodeJpeg:0': imageArray})
